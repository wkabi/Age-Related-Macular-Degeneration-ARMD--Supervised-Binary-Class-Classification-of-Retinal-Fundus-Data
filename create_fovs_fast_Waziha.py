...
parser.add_argument('--im_path_out', type=str, default='data/cropped_images_waziha/cropped_images_woenh/', help='path data without enhancement')
parser.add_argument('--im_path_out_enh', type=str, default='data/cropped_images_waziha/cropped_images_wenh/', help='path data with enhancement')
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(im_list))):
        ...
        img_clahe = cv2.imread(osp.join(im_path_out,im_name))
        #print('image shape is:', img_clahe.shape)
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
        img_1 = clahe.apply(img_clahe[:,:,0]) * 1
        img_2 = clahe.apply(img_clahe[:,:,1]) * 1
        img_3 = clahe.apply(img_clahe[:,:,2]) * 1
